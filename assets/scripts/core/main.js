function checkForAutoLoad() {
  const assetsLoaded = localStorage.getItem('splitdash_assets_loaded') === 'true';
  const lastLoadTime = parseInt(localStorage.getItem('splitdash_last_load_time') || '0');
  const now = Date.now();
...
window.clearGameCache = () => {
  if (window.gameCache) {
    ...
    localStorage.removeItem('splitdash_assets_loaded');
    localStorage.removeItem('splitdash_last_load_time');
