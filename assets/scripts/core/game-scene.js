...
this._menuFsBtn = this.add.image(33, 33, "GJ_WebSheet", _0x28fa5b ? "toggleFullscreenOff_001.png" : "toggleFullscreenOn_001.png").setScrollFactor(0).setDepth(30).setScale(0.64).setAlpha(0.8).setTint(Phaser.Display.Color.GetColor(255, 255, 255)).setInteractive();
    ...
    this._openCreatorMenu = () => {
      ...
      menuButtons.forEach((frame, idx) => {
        ...
        if (isSearchButton) {
          ...
          this._makeBouncyButton(btn, btnScale, () => {
            this._closeCreatorMenu(true);
            this._playDailyLevel(false);
          }, () => true);
        } else if (isWeeklyButton) {
          btn.setInteractive();
          this._makeBouncyButton(btn, btnScale, () => {
            this._closeCreatorMenu(true);
            this._playDailyLevel(true);
          }, () => true);
        } else {
    ...
    this._playDailyLevel = async (weekly) => {
      const PROXY = (window._gdProxyUrl || '').replace(/\/$/, '');
      if (!PROXY) return;
      try {
        const dlRes = await fetch(`${PROXY}/getGJDailyLevel.php`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: `secret=Wmfd2893gb7&weekly=${weekly ? '1' : '0'}`
        });
        const dlText = await dlRes.text();
        if (!dlText || dlText === '-1' || !dlText.includes('|')) {
          this._showAlertPopup('Daily Level', weekly ? 'No weekly level available.' : 'No daily level available.', 'OK');
          return;
        }
        const levelId = dlText.split('|')[0];
        const ldRes = await fetch(`${PROXY}/downloadGJLevel22.php`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: `levelID=${levelId}&secret=Wmfd2893gb7`
        });
        if (!ldRes.ok) throw new Error(`HTTP ${ldRes.status}`);
        const rawResponse = await ldRes.text();
        if (!rawResponse || rawResponse === '-1' || !rawResponse.includes(':')) {
          this._showAlertPopup('Daily Level', 'Failed to download level.', 'OK');
          return;
        }
        const gdMap = {};
        ...
        const levelName = gdMap["2"] || "Daily Level";
        const songIdRaw = (gdMap["35"] || "").trim();
        const isCustomSong = !!songIdRaw && songIdRaw !== "0";
        const officialSongId = gdMap["12"] || "0";
        const songKey = isCustomSong ? `ng_song_${songIdRaw}` : window.allLevels[officialSongId]?.[0] || "stereo_madness";
        ...
        console.error('Daily level failed:', e);
        this._showAlertPopup('Daily Level', 'Failed to load daily level.', 'OK');
      }
    };
        ...
        fileInput.onchange = (e) => {
            ...
            reader.onload = (event) => {
                ...
                try {
                    ...
                    this._showAlertPopup('Import Failed', err.message);
                }
            };
            reader.readAsText(file);
        };
    ...
    this._exportGMD = (level) => {
        ...
        const authorName = "Split Dashers";
    ...
    this._uploadLevel = async (level) => {
      const base = (window._gdProxyUrl || '').replace(/\/$/, '');
      const user = window.AccountAPI && window.AccountAPI.currentUser;
      if (!user) { this._showAlertPopup('Upload', 'You must be logged in to upload levels.'); return; }
      const gjp = window.AccountAPI._gjp(user.password || '');
      const levelString = level.levelString || '';
      const originalId = (level.levelId && level.levelId !== 'NA') ? parseInt(level.levelId.replace(/\D/g, '')) : 0;
      const descB64 = btoa(level.description || '').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const seed2Gen = (data) => {
        if (!data || data.length < 50) return data || '';
        const step = Math.floor(data.length / 50);
        let s = '';
        for (let i = 0; i < 50; i++) s += data[step * i];
        return s + 'xI25fpAapCQg';
      };
      const seed2Raw = seed2Gen(levelString);
      const seed2Digest = await crypto.subtle.digest('SHA-1', new TextEncoder().encode(seed2Raw));
      const seed2Hex = Array.from(new Uint8Array(seed2Digest)).map(b => b.toString(16).padStart(2, '0')).join('');
      const xorKey = '41274';
      let xored = '';
      for (let i = 0; i < seed2Hex.length; i++) {
        xored += String.fromCharCode(seed2Hex.charCodeAt(i) ^ xorKey.charCodeAt(i % xorKey.length));
      }
      const seed2 = btoa(xored).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
      const body = Object.entries({
        gameVersion: 21, accountID: user.accountID, gjp, userName: user.username,
        levelID: originalId, levelName: level.levelName, levelDesc: descB64,
        levelVersion: level.version || 1, levelLength: level.levelLength || 0,
        audioTrack: 0, auto: 0, password: 0, original: 0, twoPlayer: 0,
        songID: level.songId > 0 ? level.songId : 0, objects: 0, coins: 0,
        requestedStars: 0, unlisted: 0, ldm: 0, levelString, seed2, secret: 'Wmfd2893gb7'
      }).map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join('&');
      try {
        const res = await fetch(`${base}/uploadGJLevel21.php`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body
        });
        const text = await res.text();
        if (text && text !== '-1' && !isNaN(parseInt(text))) {
          level.status = 'Uploaded';
          level.levelId = text;
          level.createdId = text;
          const raw = localStorage.getItem('created_levels');
          const all = raw ? JSON.parse(raw) : [];
          const idx = all.findIndex(l => l.createdId === level.createdId || l.levelId === level.levelId);
          if (idx !== -1) { all[idx] = level; localStorage.setItem('created_levels', JSON.stringify(all)); }
          this._showAlertPopup('Uploaded', `Level uploaded! ID: ${text}`);
        } else {
          this._showAlertPopup('Upload Failed', text || 'unknown error');
        }
      } catch (e) {
        this._showAlertPopup('Upload Error', e.message);
      }
    };
    ...
    this._openLevelView = (level) => {
        ...
        this._makeBouncyButton(shareBtn, 1.1, () => { this._uploadLevel(level); });
    ...
    this._openSearchMenu = () => {
      ...
      const resultContainer = this.add.container(0, 0).setScrollFactor(0).setDepth(106);
      const resultMaskShape = this.add.graphics().setScrollFactor(0);
      resultMaskShape.fillStyle(0xffffff);
      resultMaskShape.fillRect(panelLeft, qsPanelY, panelW, qsPanelH);
      const resultMask = resultMaskShape.createGeometryMask();
      resultContainer.setMask(resultMask);
      this._searchOverlayObjects.push(resultMaskShape);

      let resultRows = [];
      let resultScrollY = 0;
      let dragStartRY = 0;
      let dragStartScrollR = 0;

      const _clearResults = () => {
        for (const r of resultRows) r.forEach(o => { if (o && o.destroy) o.destroy(); });
        resultRows = [];
        resultScrollY = 0;
      };
      const _buildResults = (results) => {
        searchResults = results;
        _clearResults();
        const rowH = 64;
        const startY = qsPanelY + 10;
        results.forEach((r, i) => {
          const ry = startY + i * rowH - resultScrollY;
          const row = [];
          let div = null;
          if (i > 0) {
            div = this.add.rectangle(panelLeft + panelW / 2, ry - 2, panelW - 20, 1, 0xffffff, 0.15).setScrollFactor(0).setDepth(106);
            row.push(div);
          }
          const nameTxt = this.add.bitmapText(panelLeft + 15, ry + 8, 'bigFont', r.name, 22).setOrigin(0, 0).setScrollFactor(0).setDepth(106);
          const authorTxt = this.add.bitmapText(panelLeft + 15, ry + 34, 'goldFont', `by ${r.author}`, 18).setOrigin(0, 0).setScrollFactor(0).setDepth(106);
          if (r.authorAccountID) {
            authorTxt.setInteractive();
            authorTxt.on('pointerdown', () => this._showUserProfile(r.authorAccountID));
          }
          const idTxt = this.add.bitmapText(panelLeft + 260, ry + 8, 'goldFont', `ID:${r.id}`, 20).setOrigin(0, 0).setScrollFactor(0).setDepth(106);
          const infoTxt = this.add.bitmapText(panelLeft + 260, ry + 34, 'goldFont', `${r.downloads} DL`, 18).setOrigin(0, 0).setScrollFactor(0).setDepth(106);
          const starTxt = this.add.bitmapText(panelLeft + 380, ry + 8, 'goldFont', `${r.stars}★`, 20).setOrigin(0, 0).setScrollFactor(0).setDepth(106);
          const playBtn = this.add.image(panelLeft + panelW - 40, ry + 30, 'GJ_GameSheet04', 'GJ_playBtn_001.png').setScale(0.5).setScrollFactor(0).setDepth(107);
          const pFrame = this.textures.getFrame('GJ_GameSheet04', 'GJ_playBtn_001.png');
          const pW = pFrame ? pFrame.realWidth : 80;
          const pH = pFrame ? pFrame.realHeight : 80;
          playBtn.setInteractive(new Phaser.Geom.Rectangle(-pW / 2, -pH / 2, pW, pH), Phaser.Geom.Rectangle.Contains);
          row.push(nameTxt, authorTxt, idTxt, infoTxt, starTxt, playBtn);
          const hitZone = this.add.zone(panelLeft + panelW / 2, ry + rowH / 2, panelW, rowH).setScrollFactor(0).setDepth(108).setInteractive();
          row.push(hitZone);
          const _rowIdx = i;
          this._makeBouncyButton(playBtn, 0.5, () => _startLevelByResult(_rowIdx));
          row.forEach(obj => { if (obj) resultContainer.add(obj); });
          resultRows.push(row);
        });
      };
      ...
      const filtersLabel   = this.add.bitmapText(sw / 2, filtersLabelY, "bigFont", "Search Type", labelSize)
        .setScrollFactor(0).setDepth(105).setOrigin(0.5, 0.5).setTint(labelColor);
      ...
      let searchType = 0;
      let searchPage = 0;
      const typeLabels = ['Search', 'Downloaded', 'Latest', 'By ID', 'By Player', 'Mod Sent'];
      const apiTypes = [0, 1, 4, -1, 5, 8];
      const typeBtnW = 100;
      const typeGap = 10;
      const typeStartX = sw / 2 - ((typeLabels.length * (typeBtnW + typeGap) - typeGap) / 2);
      const typeBtns = typeLabels.map((label, i) => {
        const bx = typeStartX + i * (typeBtnW + typeGap);
        const by = filtersPanelY + filtersPanelH / 2;
        const btn = this.add.nineslice(bx, by, 'GJ_button01', null, typeBtnW, 40, 12, 12, 12, 12).setScrollFactor(0).setDepth(106).setInteractive();
        const blbl = this.add.bitmapText(bx, by - 2, 'goldFont', label, 16).setOrigin(0.5).setScrollFactor(0).setDepth(106);
        this._makeBouncyButton(btn, 1, () => {
          searchType = i;
          typeBtns.forEach((b, j) => b.setTint(j === i ? 0xffffff : 0x666666));
        });
        this._searchOverlayObjects.push(btn, blbl);
        return btn;
      });
      typeBtns[0].setTint(0xffffff);
      ...
      const statusLabel = this.add.bitmapText(sw / 2, extraPanelY + extraPanelH / 2, 'goldFont', 'Type a name/ID and press Search', 22)
        .setOrigin(0.5).setScrollFactor(0).setDepth(106);
      this._searchOverlayObjects.push(statusLabel);

      const prevPageBtn = this.add.image(panelLeft + 50, extraPanelY + extraPanelH / 2, 'GJ_GameSheet03', 'GJ_arrow_03_001.png')
        .setScrollFactor(0).setDepth(106).setInteractive().setVisible(false);
      const nextPageBtn = this.add.image(panelRight - 50, extraPanelY + extraPanelH / 2, 'GJ_GameSheet03', 'GJ_arrow_03_001.png')
        .setScrollFactor(0).setDepth(106).setInteractive().setVisible(false).setFlipX(true);
      this._makeBouncyButton(prevPageBtn, 1, () => _goToPage(-1));
      this._makeBouncyButton(nextPageBtn, 1, () => _goToPage(1));
      this._searchOverlayObjects.push(prevPageBtn, nextPageBtn);
      ...
      let _loading = false;
      let _allResults = [];     // flattened results across all fetched API pages
      let _fetchedPages = {};   // Set of fetched API page numbers
      let _lastQuery = '';
      let _lastApiType = 0;
      let _searchTotalAll = 0;  // total from latest API response
      const _doSearch = async () => {
        if (_loading) return;
        const query = htmlInput.value.trim();
        if (!query) return;
        _loading = true;
        _allResults = [];
        _fetchedPages = {};
        searchPage = 0;
        _lastQuery = query;
        const apiType = apiTypes[searchType] ?? searchType;
        _lastApiType = apiType;
        if (apiType === -1) {
          const id = query.replace(/\D/g, '');
          if (id) await _downloadLevelById(id);
          _loading = false;
          return;
        }
        statusLabel.setText('Searching...');
        try {
          await _fetchApiPage(0);
          _buildResults(_allResults.slice(0, 3));
          _updatePageDisplay();
        } catch (e) {
      ...
      const _goToPage = async (dir) => {
        if (_loading) return;
        const newPage = searchPage + dir;
        if (newPage < 0) return;
        const maxGlobal = Math.max(0, Math.ceil(_searchTotalAll / 3) - 1);
        if (newPage > maxGlobal) return;
        const startIdx = newPage * 3;
        const neededPage = Math.floor(startIdx / 10);
        for (let p = Math.floor(_allResults.length / 10); p <= neededPage; p++) {
          if (!_fetchedPages[p]) {
            _loading = true;
            statusLabel.setText('Searching...');
            try {
              const fetched = await _fetchApiPage(p);
              if (fetched === 0) break;
            } catch (e) {
              console.error('Search failed:', e);
              statusLabel.setText('Search failed');
        ...
        if (startIdx >= _allResults.length) return;
        searchPage = newPage;
        _buildResults(_allResults.slice(startIdx, startIdx + 3));
        _updatePageDisplay();
      };
      const _updatePageDisplay = () => {
        const maxGlobal = Math.max(0, Math.ceil(_searchTotalAll / 3) - 1);
        if (_searchTotalAll > 3) {
          statusLabel.setText(`Page ${searchPage + 1}/${maxGlobal + 1} (${_searchTotalAll} total)`);
        } else {
          statusLabel.setText(`Found ${_searchTotalAll} results`);
        }
        prevPageBtn.setVisible(searchPage > 0);
        nextPageBtn.setVisible(searchPage < maxGlobal);
      };
      const _fetchApiPage = async (apiPage) => {
        const PROXY_BASE = (window._gdProxyUrl || '').replace(/\/$/, '');
        if (!PROXY_BASE) return 0;
        const params = { secret: 'Wmfd2893gb7', page: String(apiPage), type: String(_lastApiType), gameVersion: '21', binaryVersion: '35', gdw: '0' };
        if (_lastApiType === 0 || _lastApiType === 5) params.str = _lastQuery;
        ...
        const text = await res.text();
        if (!text || text === '-1' || !text.includes(':')) return 0;
        ...
        const playerMap = {};
        const playerAccountMap = {};
        for (const ps of playerStrs) {
          const p = ps.split(':');
          if (p.length >= 2) playerMap[p[0]] = p[1];
          if (p.length >= 3) playerAccountMap[p[0]] = p[2];
        }
        const results = levelStrs.map((ls) => {
          const m = {};
          const p = ls.split(':');
          for (let i = 0; i + 1 < p.length; i += 2) m[p[i]] = p[i + 1];
          const playerId = m['6'] || '0';
          return {
            id: m['1'] || '?',
            name: m['2'] || 'Unknown',
            author: playerMap[playerId] || ('Player ' + playerId),
            authorAccountID: playerAccountMap[playerId] || null,
            downloads: m['10'] || '0',
            stars: m['18'] || '0',
          };
        });
        if (results.length === 0) return 0;
        _fetchedPages[apiPage] = true;
        _searchTotalAll = parseInt(pageInfo[0]) || 0;
        _allResults = _allResults.concat(results);
        return results.length;
      };
      const _startLevelByResult = async (idx) => {
        const r = searchResults[idx];
        if (!r) return;
        await _downloadLevelById(r.id);
      };
      const _downloadLevelById = async (levelId) => {
        const PROXY_BASE = (window._gdProxyUrl || '').replace(/\/$/, '');
        ...
        if (!rawResponse || rawResponse === "-1" || !rawResponse.includes(":")) {
          statusLabel.setText('Level not found');
          return;
        }
        const gdMap = {};
        const _gdMatches = [...rawResponse.matchAll(/(?:^|:)(\d+):/g)];
      ...
      if (window.levelID && !window.alreadydownloaded) { // if there's an ID parameter, load it directly
        ...
        _downloadLevelById(window.levelID);
      }
...
_buildSettingsPopup() {
  ...
  _showAlertPopup(title, message, btnText = 'OK', onClose) {
    if (this._alertPopup) return;
    const cx = screenWidth / 2, cy = screenHeight / 2;
    this._alertPopup = this.add.container(0, 0).setScrollFactor(0).setDepth(1000);
    const bg = this.add.rectangle(cx, cy, screenWidth, screenHeight, 0, 100/255).setInteractive();
    this._alertPopup.add(bg);
    const bounce = this.add.container(cx, cy).setScale(0);
    this._alertPopup.add(bounce);
    const pw = 380, ph = 200;
    const cr = this.textures.get('GJ_square02').source[0].width * 0.325;
    const panel = this._drawScale9(0, 0, pw, ph, 'GJ_square02', cr, 16777215, 1);
    bounce.add(panel);
    const lbl = this.add.bitmapText(0, -ph/2 + 28, 'bigFont', title, 28).setOrigin(0.5);
    bounce.add(lbl);
    const txt = this.add.text(0, -10, message, {
      fontSize: '20px', fontFamily: 'Arial, sans-serif', color: '#ffffff', align: 'center', wordWrap: { width: pw - 40 }
    }).setOrigin(0.5);
    bounce.add(txt);
    const btnCont = this.add.container(0, ph/2 - 50);
    const btnW = 140, btnH = 50;
    const btnBorder = this.textures.get('GJ_button01').source[0].width * 0.3;
    const btnBg = this._drawScale9(0, 0, btnW, btnH, 'GJ_button01', btnBorder, 0xffffff, 1);
    const btnHit = this.add.rectangle(0, 0, btnW, btnH).setInteractive();
    btnCont.add(btnBg);
    btnCont.add(btnHit);
    const btnLbl = this.add.bitmapText(-2, -3, 'goldFont', btnText, 36).setOrigin(0.5);
    btnCont.add(btnLbl);
    bounce.add(btnCont);
    btnHit.on('pointerdown', () => { this.tweens.add({ targets: btnCont, scaleX: 1.2, scaleY: 1.2, duration: 150 }); });
    btnHit.on('pointerup', () => {
      this.tweens.killTweensOf(btnCont);
      btnCont.setScale(1);
      if (this._alertPopup) { this._alertPopup.destroy(); this._alertPopup = null; }
      if (onClose) onClose();
    });
    this.tweens.add({ targets: bounce, scale: { from: 0, to: 1 }, duration: 660, ease: 'Elastic.Out', easeParams: [1, 0.6] });
  }
  async _buildAccountPopup() {
    if (this._accountPopup) return;
    const cx = screenWidth / 2, cy = 320;
    this._accountPopup = this.add.container(0, 0).setScrollFactor(0).setDepth(1000);
    const bg = this.add.rectangle(cx, cy, screenWidth, screenHeight, 0, 100/255).setInteractive();
    this._accountPopup.add(bg);
    const container = this.add.container(cx, cy).setScale(0);
    this._accountPopup.add(container);
    const corner = 0.325 * this.textures.get('GJ_square02').source[0].width;
    const panel = this._drawScale9(0, 0, 480, 400, 'GJ_square02', corner, 16777215, 1);
    container.add(panel);
    const closeBtn = this.add.image(-230, -175, 'GJ_WebSheet', 'GJ_closeBtn_001.png').setScale(0.8).setInteractive();
    container.add(closeBtn);
    this._expandHitArea(closeBtn, 2);
    this._makeBouncyButton(closeBtn, 0.8, () => { this._accountPopup.destroy(); this._accountPopup = null; });
    const title = this.add.bitmapText(0, -155, 'bigFont', 'Account', 35).setOrigin(0.5);
    container.add(title);
    const user = window.AccountAPI.currentUser;
    if (user) {
      const nameTxt = this.add.bitmapText(0, -120, 'bigFont', user.username, 30).setOrigin(0.5);
      container.add(nameTxt);
      const statusLbl = this.add.bitmapText(0, -80, 'goldFont', 'Account ID: ' + user.accountID, 22).setOrigin(0.5);
      container.add(statusLbl);
      const profileBtn = this.add.nineslice(-90, 0, 'GJ_button01', null, 180, 60, 24, 24, 24, 24).setInteractive();
      container.add(profileBtn);
      const profileLbl = this.add.bitmapText(-90, -2, 'bigFont', 'Profile', 28).setOrigin(0.5);
      container.add(profileLbl);
      this._makeBouncyButton(profileBtn, 1, () => {
        this._accountPopup.destroy();
        this._accountPopup = null;
        this._buildProfilePopup(user.accountID);
      });
      const lbBtn = this.add.nineslice(90, 0, 'GJ_button01', null, 180, 60, 24, 24, 24, 24).setInteractive();
      container.add(lbBtn);
      const lbLbl = this.add.bitmapText(90, -2, 'bigFont', 'Leaderboard', 28).setOrigin(0.5);
      container.add(lbLbl);
      this._makeBouncyButton(lbBtn, 1, () => {
        this._accountPopup.destroy();
        this._accountPopup = null;
        this._buildLeaderboardPopup();
      });
      const logoutBtn = this.add.nineslice(0, 80, 'GJ_button01', null, 260, 60, 24, 24, 24, 24).setInteractive();
      container.add(logoutBtn);
      const logoutLbl = this.add.bitmapText(0, 78, 'bigFont', 'Logout', 28).setOrigin(0.5);
      container.add(logoutLbl);
      this._makeBouncyButton(logoutBtn, 1, () => {
        window.AccountAPI.logout();
        this._accountPopup.destroy();
        this._accountPopup = null;
      });
    } else {
      const nameInput = this.add.bitmapText(0, -100, 'bigFont', 'Username', 22).setOrigin(0.5);
      container.add(nameInput);
      const nameBg = this.add.graphics();
      nameBg.fillStyle(0x000000, 0.3);
      nameBg.fillRoundedRect(-150, -80, 300, 50, 8);
      container.add(nameBg);
      const nameField = this.add.bitmapText(-140, -65, 'bigFont', '', 26).setOrigin(0, 0.5);
      container.add(nameField);
      const nameHint = this.add.bitmapText(-140, -65, 'bigFont', 'Enter username...', 26).setOrigin(0, 0.5).setTint(0x666666);
      container.add(nameHint);
      const passInput = this.add.bitmapText(0, -25, 'bigFont', 'Password', 22).setOrigin(0.5);
      container.add(passInput);
      const passBg = this.add.graphics();
      passBg.fillStyle(0x000000, 0.3);
      passBg.fillRoundedRect(-150, -5, 300, 50, 8);
      container.add(passBg);
      const passField = this.add.bitmapText(-140, 10, 'bigFont', '', 26).setOrigin(0, 0.5);
      container.add(passField);
      const passHint = this.add.bitmapText(-140, 10, 'bigFont', 'Enter password...', 26).setOrigin(0, 0.5).setTint(0x666666);
      container.add(passHint);
      let inputMode = 'user';
      let userName = '', password = '';
      const updateFields = () => {
        nameField.setText(userName);
        nameHint.setVisible(!userName);
        passField.setText('*'.repeat(password.length));
        passHint.setVisible(!password);
      };
      const nameHit = this.add.zone(-140, -55, 280, 40).setInteractive();
      container.add(nameHit);
      const passHit = this.add.zone(-140, 20, 280, 40).setInteractive();
      container.add(passHit);
      nameHit.on('pointerdown', () => { inputMode = 'user'; });
      passHit.on('pointerdown', () => { inputMode = 'pass'; });
      const keyHandler = (e) => {
        if (!this._accountPopup) { window.removeEventListener('keydown', keyHandler); return; }
        if (e.key === 'Backspace') {
          if (inputMode === 'user') userName = userName.slice(0, -1);
          else password = password.slice(0, -1);
        } else if (e.key === 'Enter') {
          if (userName && password) doLogin();
        } else if (e.key.length === 1) {
          if (inputMode === 'user' && userName.length < 20) userName += e.key;
          else if (inputMode === 'pass' && password.length < 30) password += e.key;
        }
        updateFields();
      };
      window.addEventListener('keydown', keyHandler);
      container.on('destroy', () => window.removeEventListener('keydown', keyHandler));
      const statusText = this.add.bitmapText(0, 70, 'bigFont', '', 20).setOrigin(0.5).setTint(0xff6666);
      container.add(statusText);
      const doLogin = async () => {
        statusText.setText('Logging in...').setTint(0xffff00);
        try {
          await window.AccountAPI.login(userName, password);
          this._accountPopup.destroy();
          this._accountPopup = null;
        } catch (e) {
          statusText.setText(e.message || 'Login failed').setTint(0xff6666);
        }
      };
      const doRegister = () => {
        window.open('https://split.ps.fhgdps.com/dashboard/login/register.php', '_blank');
      };
      const loginBtn = this.add.nineslice(-80, 110, 'GJ_button01', null, 150, 60, 24, 24, 24, 24).setInteractive();
      container.add(loginBtn);
      const loginLbl = this.add.bitmapText(-80, 108, 'bigFont', 'Login', 28).setOrigin(0.5);
      container.add(loginLbl);
      this._makeBouncyButton(loginBtn, 1, doLogin);
      const regBtn = this.add.nineslice(80, 110, 'GJ_button01', null, 150, 60, 24, 24, 24, 24).setInteractive();
      container.add(regBtn);
      const regLbl = this.add.bitmapText(80, 108, 'bigFont', 'Register', 28).setOrigin(0.5);
      container.add(regLbl);
      this._makeBouncyButton(regBtn, 1, doRegister);
      container.add(nameInput);
      container.add(passInput);
    }
    this.tweens.add({ targets: container, scale: { from: 0, to: 1 }, duration: 660, ease: 'Elastic.Out', easeParams: [1, 0.6] });
  }

  async _buildProfilePopup(accountID) {
    if (this._profilePopup) return;
    const cx = screenWidth / 2, cy = 320;
    this._profilePopup = this.add.container(0, 0).setScrollFactor(0).setDepth(1000);
    const bg = this.add.rectangle(cx, cy, screenWidth, screenHeight, 0, 100/255).setInteractive();
    this._profilePopup.add(bg);
    const container = this.add.container(cx, cy).setScale(0);
    this._profilePopup.add(container);
    const corner = 0.325 * this.textures.get('GJ_square02').source[0].width;
    const panel = this._drawScale9(0, 0, 480, 400, 'GJ_square02', corner, 16777215, 1);
    container.add(panel);
    const closeBtn = this.add.image(-230, -175, 'GJ_WebSheet', 'GJ_closeBtn_001.png').setScale(0.8).setInteractive();
    container.add(closeBtn);
    this._expandHitArea(closeBtn, 2);
    this._makeBouncyButton(closeBtn, 0.8, () => { this._profilePopup.destroy(); this._profilePopup = null; });
    const loadingTxt = this.add.bitmapText(0, -60, 'bigFont', 'Loading...', 28).setOrigin(0.5);
    container.add(loadingTxt);
    this.tweens.add({ targets: container, scale: { from: 0, to: 1 }, duration: 660, ease: 'Elastic.Out', easeParams: [1, 0.6] });
    try {
      const info = await window.AccountAPI.getUserInfo(accountID);
      if (!this._profilePopup) return;
      loadingTxt.destroy();
      const title = this.add.bitmapText(0, -155, 'bigFont', info.username, 35).setOrigin(0.5);
      container.add(title);
      const stats = [
        `Stars: ${info.stars}`,
        `Demons: ${info.demons}`,
        `Coins: ${info.secretCoins}`,
        `User Coins: ${info.userCoins}`,
        `Diamonds: ${info.diamonds}`,
        `Creator Points: ${info.creatorPoints}`,
      ];
      if (info.globalRank !== '0') stats.push(`Global Rank: ${info.globalRank}`);
      if (info.ranking !== '0') stats.push(`Leaderboard Rank: ${info.ranking}`);
      const badge = parseInt(info.modLevel) === 2 ? '[Elder Mod]' : parseInt(info.modLevel) === 1 ? '[Mod]' : '';
      if (badge) stats.push(badge);
      const startY = -90;
      stats.forEach((s, i) => {
        const t = this.add.bitmapText(0, startY + i * 32, 'goldFont', s, 22).setOrigin(0.5);
        container.add(t);
      });
      const linkY = startY + stats.length * 32 + 10;
      const links = [];
      if (info.youtube) links.push({ label: 'YouTube', url: info.youtube });
      if (info.twitter) links.push({ label: 'Twitter', url: info.twitter });
      if (info.twitch) links.push({ label: 'Twitch', url: info.twitch });
      links.forEach((link, i) => {
        const t = this.add.bitmapText(0, linkY + i * 28, 'goldFont', link.label, 20).setOrigin(0.5).setInteractive().setTint(0x00aaff);
        container.add(t);
        t.on('pointerdown', () => window.open('https://' + link.url.replace(/^https?:\/\//, ''), '_blank'));
      });
    } catch (e) {
      if (this._profilePopup) loadingTxt.setText('Failed to load profile').setTint(0xff6666);
    }
  }

  async _buildLeaderboardPopup() {
    if (this._leaderboardPopup) return;
    const cx = screenWidth / 2, cy = 320;
    this._leaderboardPopup = this.add.container(0, 0).setScrollFactor(0).setDepth(1000);
    const bg = this.add.rectangle(cx, cy, screenWidth, screenHeight, 0, 100/255).setInteractive();
    this._leaderboardPopup.add(bg);
    const container = this.add.container(cx, cy).setScale(0);
    this._leaderboardPopup.add(container);
    const corner = 0.325 * this.textures.get('GJ_square02').source[0].width;
    const panel = this._drawScale9(0, 0, 520, 440, 'GJ_square02', corner, 16777215, 1);
    container.add(panel);
    const closeBtn = this.add.image(-255, -205, 'GJ_WebSheet', 'GJ_closeBtn_001.png').setScale(0.8).setInteractive();
    container.add(closeBtn);
    this._expandHitArea(closeBtn, 2);
    this._makeBouncyButton(closeBtn, 0.8, () => { this._leaderboardPopup.destroy(); this._leaderboardPopup = null; });
    const title = this.add.bitmapText(0, -185, 'bigFont', 'Leaderboard', 32).setOrigin(0.5);
    container.add(title);
    const loadingTxt = this.add.bitmapText(0, -60, 'bigFont', 'Loading...', 28).setOrigin(0.5);
    container.add(loadingTxt);
    this.tweens.add({ targets: container, scale: { from: 0, to: 1 }, duration: 660, ease: 'Elastic.Out', easeParams: [1, 0.6] });
    try {
      const entries = await window.AccountAPI.getLeaderboard('top', 50);
      if (!this._leaderboardPopup) return;
      loadingTxt.destroy();
      const rowH = 36;
      const startY = -140;
      const maxRows = Math.min(entries.length, 14);
      for (let i = 0; i < maxRows; i++) {
        const e = entries[i];
        const ry = startY + i * rowH;
        const rankStr = `#${i + 1}`;
        const nameStr = e.username.substring(0, 16);
        const statStr = `★${e.stars} 💎${e.diamonds} D:${e.demons}`;
        const rankTxt = this.add.bitmapText(-230, ry, 'goldFont', rankStr, 20).setOrigin(0, 0.5);
        const nameTxt = this.add.bitmapText(-170, ry, 'bigFont', nameStr, 20).setOrigin(0, 0.5);
        const statTxt = this.add.bitmapText(60, ry, 'goldFont', statStr, 18).setOrigin(0, 0.5);
        container.add(rankTxt);
        container.add(nameTxt);
        container.add(statTxt);
        const hit = this.add.zone(-100, ry, 200, rowH).setInteractive();
        container.add(hit);
        const aid = e.accountID;
        hit.on('pointerdown', () => {
          this._leaderboardPopup.destroy();
          this._leaderboardPopup = null;
          this._buildProfilePopup(aid);
        });
      }
    } catch (e) {
      loadingTxt.setText('Failed to load leaderboard').setTint(0xff6666);
    }
  }

  _showUserProfile(targetAccountID) {
    this._buildProfilePopup(targetAccountID);
  }
...
_decodeLevelString(encoded) {
  try {
    let s = encoded;
    if (s.startsWith('H4sI')) { /* already correct */ }
...
_applyMirrorEffect() {
  ...
  _showSettingsScreen() {
    ...
    _makeSettingsBtn(_sColR, _sRow2Y, "Graphics",   _sBtnW2, true,  () => { this._buildSettingsPopup(); });
    _makeSettingsBtn(_sCol3L, _sRow3Y, "Leaderboard", _sBtnW3, true,  () => { this._buildLeaderboardPopup(); });
    _makeSettingsBtn(_sCol3M, _sRow3Y, "Songs",     _sBtnW3, true,  () => { window.open('https://www.newgrounds.com/audio', '_blank'); });
    _makeSettingsBtn(_sCol3R, _sRow3Y, "Help",      _sBtnW3, true,  () => { this._buildHowToPlayPopup(); });
    ...
    const _setPage = async (page) => {
      ...
      try {
        ...
        if (!response) {
          ...
          const body = Object.entries({ secret: "Wmfd2893gb7", page, gameVersion: "21", binaryVersion: "35", gdw: "0", ...params })
            .map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join("&");
          let retryCount = 0;
          const maxRetries = 3;
          let res;
